...
class TrafficSignalController:
    ...
    def __init__(self, conn, tsc_id, mode, netdata, red_t, yellow_t):
        ...
        if mode == 'train':
            self.metric_args = ['queue', 'delay', 'emission', 'waitingtime', 'throughput', 'speed']
        if mode == 'test':
            self.metric_args = ['queue', 'delay', 'emission', 'waitingtime', 'throughput', 'speed']
        self.trafficmetrics = TrafficMetrics(tsc_id, self.incoming_lanes, netdata, self.metric_args, mode, self.conn)
    ...
    def get_state(self):
        # Richer state: density + queue + avg_wait + avg_speed + current phase one-hot
        density = self.get_normalized_density()
        queue   = self.get_normalized_queue()
        wait    = self.get_normalized_waiting_time()
        speed   = self.get_normalized_speed()
        phase   = self.get_phase_one_hot()

        state = np.concatenate([density, queue, wait, speed, phase])

        if not hasattr(self, "_printed_state_shape"):
            print("STATE SHAPE =", state.shape)
            self._printed_state_shape = True

        return state

    def get_normalized_waiting_time(self):
        wait_times = []
        ...
        for lane in self.incoming_lanes:
            ...
            for v in self.data[lane]:
                try:
                    total_wait += self.conn.vehicle.getWaitingTime(v)
                except:
                    pass

            cap = self.lane_capacity[self.incoming_lanes.index(lane)]
            wait_times.append(total_wait / (cap * 60.0))
    ...
    def get_normalized_speed(self):
        ...
        for lane in self.incoming_lanes:
            if len(self.data[lane]) == 0:
                speeds.append(1.0)  # free flow if empty
            else:
                avg = np.mean([
                    self.data[lane][v][traci.constants.VAR_SPEED]
                    for v in self.data[lane]
                ])
                max_speed = self.netdata['lane'][lane]['speed']
                speeds.append(avg / max_speed)
        return np.array(speeds)

    def get_phase_one_hot(self):
        one_hots = self.input_to_one_hot(self.green_phases)
        return one_hots.get(self.phase, np.zeros(len(self.green_phases)))

    def get_reward(self):
        delay = self.trafficmetrics.get_metric('delay')
        queue = self.trafficmetrics.get_metric('queue')
        emission = self.trafficmetrics.get_metric('emission')
        waiting = self.trafficmetrics.get_metric('waitingtime')
        throughput = self.trafficmetrics.get_metric('throughput')
        speed = self.trafficmetrics.get_metric('speed')
        # normalization
        delay = delay / 100.0
        queue = queue / 10.0
        co2 = emission['co2'] / 10000.0
        waiting = waiting / 1000.0
        throughput = throughput / 20.0
        speed = speed / 15.0
        # reward 
        reward = (-0.30*delay - 0.20*queue - 0.15*co2 - 0.15*waiting + 0.10*throughput + 0.10*speed)
