...
def graph(ax, data, graph_func, xtitle=None, ytitle_pad=None, title=None, legend=None, grid=None, xlim=None, ylim=None, colours=None):
    ...
    if ytitle_pad:
        if isinstance(ytitle_pad, tuple):
            ax.set_ylabel(ytitle_pad[0])
        else:
            ax.set_ylabel(ytitle_pad)
    ...
    if legend:
        if colours is None:
            ax.legend(elements, labels, loc=legend, framealpha=0.9)
        else:
            patches = [ mpatches.Patch(color=colours[l], label=l) for l in labels]
            ax.legend(handles=patches, loc=legend, framealpha=0.9)
...
def boxplot(ax, data, colors, data_labels):
    ...
    for b, c in zip(bp['medians'], colors):
        b.set(color='w', linewidth=2)

    for b, c in zip(bp['boxes'], colors):
        b.set(facecolor=c, edgecolor='k', linewidth=0.8, alpha=0.85)

    for b, c in zip(bp['fliers'], colors):
        b.set(marker='o', markeredgecolor=c, markerfacecolor='none',
...
def save_graph(f, fname, dpi=300, h=5, w=8):
    f.set_size_inches(w, h, forward=True)
    f.savefig(fname, dpi=dpi, bbox_inches='tight', pad_inches=0.1)
