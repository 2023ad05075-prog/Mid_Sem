...
class DDPGCriticNet:
    def __init__(self, input_d, hidden_d, hidden_act, action_d, output_act, lr, lre, name, sess):
        with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            dense3 = tf.compat.v1.layers.dense(state_actions, units=hidden_d[1],
                                     kernel_initializer=HeUniform())
            batch3 = tf.compat.v1.layers.batch_normalization(dense3, training=self.is_training)
    ...
    def gradients(self, states, actions):
        return self.sess.run(self.models['online'].action_gradients,
                             feed_dict={self.models['online'].input: states,
                                        ...
                                        self.models['online'].is_training: False})
