...
# Must be set before importing tensorflow so tf.compat.v1.layers works with Keras 3
...
def nn_factory( nntype, input_d, output_d, args, learner, load, tsc, n_hidden, sess=None):
    ...
    if nntype == 'dqn':
        ...
        nn = {}
        # PPO actor outputs logits over actions
        nn['actor'] = DQN(input_d, hidden_layers,
                          args.hidden_act, output_d,
                          'linear', args.lr,
                          args.lre, learner=learner)
        # PPO critic outputs single value
        nn['critic'] = DQN(input_d, hidden_layers,
                           args.hidden_act, 1,
                           'linear', args.lrc,
                           args.lre, learner=learner)
    else:
...
def get_in_out_d(tsctype, n_incoming_lanes, n_phases):
    # state = [density, queue, wait, speed] (4 * n_lanes) + phase_one_hot (n_phases)
    input_d = (n_incoming_lanes * 4) + n_phases
    if tsctype == 'dqn' or tsctype == 'ppo':
        return input_d, n_phases
    elif tsctype == 'ddpg':
        # NextDurationRLTSC appends next-phase one-hot (n_phases + 1 for all_red)
        return input_d + n_phases + 1, 1
    else:
